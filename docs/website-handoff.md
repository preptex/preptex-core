...
uses that same exact dependency. C0–C9, including C5a/C7a, are implemented. C10's local verification is
...
The [node-operation contract](./node-operations.md) finalizes source environment
inventory, line/interval lookup, revision-bound selections, removal, renaming and
wrapping. Website W4a/W6a can use the exported types/helpers once the exact package
is published. Keep source buffers separate from generated artifacts, invalidate
selections after edits/configuration changes, and surface located capability
failures when shared-source edits cannot represent requested inclusion edits.
...
no skipped library checks, and no Node/DOM ambient types. All three public type suites
run against the installed declarations. The script also checks ESM exports and
blocked deep imports, executes both public examples, all four public project
...
The 0.3.0 verification covers 377 core tests and nine CLI tests in the checkout;
210 public project acceptance tests, all three public declaration suites and the nine
...
| F31      | `node-operations.test.ts` and `F31-node-removal.tex`: whole section/environment/group removal, unchanged neighbors and frozen inputs                                                                                                                  |
| F32–F33  | `node-operations.test.ts`: nested/both-branch source environments, protected/stored syntax, malformed coverage, source flags and explicit combined comment suppression                                                                                |
| F34–F36  | `node-operations.test.ts`: exact two-name list rename, ordered wrappers, EOF lexical joins, synthetic provenance, disjoint/multi-file and shared-inclusion conflicts                                                                                  |
| F37–F38  | `node-operations.test.ts`: Unicode and line offsets, empty/disjoint/repeated indexed lookup, stale/transported selections, normalized batches, pipeline ordering and bounds                                                                           |
