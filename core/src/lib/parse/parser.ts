...
import { Lexer, TokenType } from './tokens.js';
...
export class Parser {
  ...
  private registerDefaultHandlers() {
    ...
    this.handlers.set(TokenType.ConditionDeclaration, this.handleConditionDeclaration.bind(this));
  }
  ...
  private handleConditionDeclaration(_token: any) {
    const token = _token as any;
    const parent = this.getParentNode() as any;
    if (!parent) throw new Error('Stack empty');
    parent.children.push({
  ...
  export(ast: AstRoot, _options: CoreOptions): JSON {
